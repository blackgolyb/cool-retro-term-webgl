...
export class TerminalText {
	...
	constructor(width: number, height: number) {
		...
		// Load noise texture from embedded base64
		const textureLoader = new THREE.TextureLoader();
		this.noiseTexture = textureLoader.load(NOISE_TEXTURE_BASE64);
		...
		// Create dynamic shader material (Pass 2)
		...
		// Create plane geometry for main mesh
	...
	private async loadFont(): Promise<void> {
		try {
			const font = new FontFace("Terminus", `url(${TERMINUS_FONT_BASE64})`);
			await font.load();
